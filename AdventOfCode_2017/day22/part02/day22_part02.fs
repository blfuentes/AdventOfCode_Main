    ...
    let rec burst ((c, d): (int*int)*Direction) (currentBurst: int) (numInfections: int)=
        if currentBurst = numOfBurst then
            ...
            let ((dr, dc), newdir), newinfections =
                ...
                elif currentnode.IsClean then
                    map[c] <- Weakened
                    (turn d Left, numInfections)
                elif currentnode.IsWeakened then
                    map[c] <- Infected
                    ((diffOfDir d, d), numInfections+1)
                else
                    map[c] <- Clean
                    (reverse d, numInfections)
...
let execute() =
    ...
    startBurst map (initcarrier, dir) 10000000