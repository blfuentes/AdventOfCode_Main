...
let BugLightmap(lightmap: Coord[,]) =
    let (maxrows, maxcols) = (lightmap.GetLength(0), lightmap.GetLength(1))

    lightmap[0, 0] <- { lightmap[0, 0] with State = ON }
    lightmap[0, maxcols-1] <- { lightmap[0, maxcols-1] with State = ON }
    lightmap[maxrows-1, 0] <- { lightmap[maxrows-1, 0] with State = ON }
    lightmap[maxrows-1, maxcols-1] <- { lightmap[maxrows-1, maxcols-1] with State = ON }
    ...
    let newstates =
        [for row in 0..maxrows-1 do
            for col in 0..maxcols-1 do
                ...
                BugLightmap lightmap

                let states = 
                    neigh 
                    |> List.map(fun (r, c) -> lightmap[r, c])
                    |> List.groupBy(fun lm -> lm.State)
                match states |> List.tryFind(fun (s, _) -> s.IsON)  with
                | Some(s) -> lightstates[ON] <- lightstates[ON] + (snd s).Length
                | _ -> ()
                match states |> List.tryFind(fun (s, _) -> s.IsOFF)  with
