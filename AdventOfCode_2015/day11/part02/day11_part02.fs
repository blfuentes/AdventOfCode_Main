...
let rec findPassword(value: char list) (findadditional: bool): char list =
    if isValidPassword value then
        if findadditional then findPassword (increment value) false else  value
    else
        findPassword (increment value) findadditional

let execute =
    let input = "vzbxkghb"
    let result = findPassword (increment(input.ToCharArray() |> List.ofArray)) true
