...
let rec evaluate (instructions: Operation list) (currentMap: Dictionary<string, uint16>) =
    ...
    | op :: tail ->
        let remainingsIns' =
            ...
            | _ ->
                if isWired op.op1 currentMap then
                    currentMap[op.result] <- value op.op1 currentMap
                    []
                else
                    [op]

        evaluate (tail @ remainingsIns') currentMap
