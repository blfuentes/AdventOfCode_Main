...
let rec sequenceOfKeys(input: Map<string, Coord>)((code, robot): string*int) (mymemo: Dictionary<string,int>): int =
    ...
    | _ ->
        ...
        |> List.iter(fun idx ->
            ...
            if robot = 0 then
                len <- len + moves[0].Length
            else
                let minlen = 
                    moves
                    |> List.fold (fun acc mov -> min acc (sequenceOfKeys directions (mov, robot - 1) mymemo)) System.Int32.MaxValue
...
let sumOfComplexity (content: string array) (complexitydepth: int) : int =
    let rec sumOfComplexityHelper (content: string array) (complexitydepth: int) (index: int) (acc: int) : int =
        if index >= content.Length then
            acc
        else
            let code = content[index]
            let lenb = sequenceOfKeys keypositions (code, complexitydepth) (Dictionary<string, int>())
            let numValue = int(code.Substring(0, 3))
            let newAcc = acc + lenb * numValue
            sumOfComplexityHelper content complexitydepth (index + 1) newAcc

    sumOfComplexityHelper content complexitydepth 0 0

let execute() =
    let path = "day21/day21_input.txt"

    let content = LocalHelper.GetLinesFromFile path
    sumOfComplexity content 2