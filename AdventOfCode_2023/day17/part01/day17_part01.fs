...
let travel (grid: int[,]) (maxSteps: int) =
    ...
    while(queue.Count > 0) do
        ...
        if (numOfMoves < maxSteps) then
            doStep grid visited queue row col direction heat numOfMoves maxSteps
        if (numOfMoves > 0) then
            doStep grid visited queue row col (turnLeft direction) heat 0 maxSteps
            doStep grid visited queue row col (turnRight direction) heat 0 maxSteps
